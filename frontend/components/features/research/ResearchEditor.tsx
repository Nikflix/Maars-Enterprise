...
import { HumanizerButton } from '../humanizer/HumanizerButton';
...
export const ResearchEditor = ({
    ...
    // Humanizer Hook
    const { isHumanizing, progress: humanizeProgress, humanizedContent, error: humanizeError, startHumanize, reset: resetHumanize } = useHumanizer();
    const [humanizeDone, setHumanizeDone] = useState(false);

    // Apply humanized content when it arrives
    useEffect(() => {
        if (humanizedContent && onContentUpdate) {
            onContentUpdate(humanizedContent);
            setHumanizeDone(true);
            handleAction('Paper humanized successfully!', () => { });
            // Reset plagiarism results so the button goes back to normal for re-checking
            resetScan();
            // Reset the done state after 5 seconds
            setTimeout(() => setHumanizeDone(false), 5000);
        }
    }, [humanizedContent]);
    ...
    useEffect(() => {
        if (humanizeError) {
            handleAction(`Error: ${humanizeError}`, () => { });
        }
    }, [humanizeError]);
    ...
    const handleFixIssues = async (analysis: string, indicators: string[]) => {
        setIsFixing(true);
        try {
            // Start fix job
            const startRes = await fetch(`${BACKEND_URL}/api/humanize/fix`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ content: paperContent, analysis, indicators }),
            });
            if (!startRes.ok) throw new Error('Failed to start fix');
            ...
            const poll = setInterval(async () => {
                try {
                    ...
                    if (statusData.status === 'completed') {
                        ...
                        if (resultData.humanized_content && onContentUpdate) {
                            onContentUpdate(resultData.humanized_content);
                            resetScan();
                            setIsReportOpen(false);
                            handleAction('Issues fixed successfully!', () => { });
                        }
                        setIsFixing(false);
                    } else if (statusData.status === 'error') {
                        clearInterval(poll);
                        setIsFixing(false);
                        handleAction(`Fix failed: ${statusData.error || 'Unknown error'}`, () => { });
                    }
                } catch {
                    clearInterval(poll);
                    setIsFixing(false);
            ...
            setIsFixing(false);
            handleAction(`Fix failed: ${err.message}`, () => { });
        }
    };
        ...
        <div className="h-screen pt-16 flex flex-col bg-black text-white overflow-hidden relative">
            ...
            <header className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-black/50 backdrop-blur-md z-10 sticky top-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <HumanizerButton
                        ...
                        isDone={humanizeDone}
                        disabled={isGenerating || !paperContent || isHumanizing}
                    />
            ...
            <div className="flex-1 overflow-hidden relative flex justify-center bg-[#050505] p-0 sm:p-8">
                <div className="w-full h-full overflow-y-auto custom-scrollbar scroll-smooth" ref={scrollRef}>
                    <div className="max-w-4xl mx-auto min-h-full bg-[#121212] border border-white/5 shadow-2xl sm:rounded-xl p-8 sm:p-16 relative overflow-hidden">
                        {/* Paper Watermark/Decoration */}
                        <div className="absolute top-0 right-0 p-6 opacity-10 pointer-events-none">
                            <FileText className="w-24 h-24 text-white" />
                        </div>

                        <article className="prose prose-invert max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-h1:text-4xl prose-h1:mb-8 prose-p:leading-relaxed prose-p:text-white/70 prose-li:text-white/70 break-words" style={{ overflowWrap: 'break-word', wordBreak: 'break-word' }}>
                            {paperContent ? (
